...
class TemplateVariable:
    ...
    name: str
    values: List[str]
    attributes: Optional[Dict] = None  # Previously called metadata: e.g., sentiment polarity, POS tag, etc.
    class_bound: bool = False  # Whether this variable must match classes with other variables
    class_key: Optional[str] = None  # Attributes key that defines the class (e.g., "sentiment")
...
class TemplatedDataset:
    ...
    def to_probing_dataset(
        self,
        label_from_attributes: Optional[str] = None,  # Previously called label_from_metadata, e.g., "sentiment"
            ...
            for class_value in possible_classes or [None]:
                ...
                for var_name in bound_vars:
                    ...
                    if class_value is not None:
                        ...
                        class_indices = [
                            i for i, v in enumerate(var.attributes[var.class_key])
                            if v == class_value
                        ]
                ...
                for bound_combo in bound_combinations:
                    for neutral_combo in neutral_combinations:
                        ...
                        for name, value in zip(bound_vars, bound_combo):
                            ...
                            if var.attributes:
                                ...
                                sliced_attr = {
                                    ...
                                    if isinstance(var.attributes[k], list) and len(var.attributes[k]) > value_index
                                }
                            var_attributes[name] = sliced_attr if sliced_attr else var.attributes
                        ...
                        label = 0
                        label_text = ""
                        if label_from_attributes and label_map and class_value is not None:
