...
class BaseProbeTrainer(ABC):
    ...
    def _get_lr_scheduler(
        ...
        if start_lr == end_lr:
            ...
            if ratio <= 0:
                print(f"Warning: Invalid learning rate range ({start_lr} -> {end_lr}). Using constant LR.")
                return optim.lr_scheduler.ConstantLR(optimizer, factor=1.0)
...
class SupervisedProbeTrainer(BaseProbeTrainer):
    ...
    def prepare_supervised_data(
        ...
        if n_train == n_total:
            n_train = max(0, n_total - 1) # Keep at least one sample for validation if possible
            if n_train == 0 and n_total > 0:
                print("Warning: Only one data point available. Using it for training and validation.")
                n_train = 1
            elif n_total > 0:
                print("Warning: train_ratio resulted in no validation data. Adjusting to keep one sample for validation.")

        # Ensure train set is not empty if n_total > 0
        if n_train == 0 and n_total > 0:
            print("Warning: train_ratio resulted in no training data. Using all data for training.")
            n_train = n_total
        ...
        if len(train_indices) == 0 and n_total > 0:
            ...
            print("Warning: No training samples after split. Using all data for training and validation.")
        elif len(val_indices) == 0 and n_total > 0:
            val_indices = train_indices # Use training data for validation if val split is empty
            print("Warning: No validation samples after split. Using training data for validation.")
        ...
        train_dataset = TensorDataset(X_train_split, y_train_split, X_orig_train_split) # Add X_orig
        val_dataset = TensorDataset(X_val_split, y_val_split, X_orig_val_split) # Add X_orig
    ...
    def train_epoch(
        ...
        for batch_x_train, batch_y, _ in batch_pbar: # Ignore X_orig during training pass
            optimizer.zero_grad()
    ...
    def train(
        ...
        if self.config.standardize_activations and self.feature_std is not None:
            ...
            with torch.no_grad():
                ...
                if learned_direction.dim() == 2 and learned_direction.shape[0] == 1 and std_dev.dim() == 1:
                ...
                elif learned_direction.dim() == 1 and std_dev.dim() == 1 and learned_direction.shape[0] == std_dev.shape[0]:
                    # Case for single vector directions (like maybe directional probes before squeeze?)
                    unscaled_direction = learned_direction / std_dev
                else:
                    print(f"Warning: Shape mismatch during final unscaling. Direction: {learned_direction.shape}, StdDev: {std_dev.shape}. Skipping unscaling.")
                    unscaled_direction = learned_direction # Keep original if shapes mismatch
...
class DirectionalProbeTrainer(BaseProbeTrainer):
    ...
    def train(
        ...
        train_loader: DataLoader,
        val_loader: Optional[DataLoader] = None, # val_loader is ignored but kept for API consistency
    ) -> Dict[str, List[float]]:
        ...
        if self.config.standardize_activations and self.feature_std is not None:
            ...
            if initial_direction.shape == std_dev.shape:
                final_direction = initial_direction / std_dev
                 ...
                 print(f"Warning: Shape mismatch during directional probe unscaling. Direction: {initial_direction.shape}, StdDev: {std_dev.shape}. Skipping unscaling.")
